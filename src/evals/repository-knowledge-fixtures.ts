...
export const repositoryKnowledgeFixtures = [
  ...
  },
  {
    ...
    expectedCapabilities: [
      capability({ key: "email_intake", label: "Conversational email intake", domainKey: "intake", matchPatterns: ["(?:email|client).{0,30}(?:intake|conversation)|conversation.{0,30}email"], evidencePathPatterns: ["conversational_responder\\.py|email[-_]intake"], exampleClaim: "Built conversational email intake that turns client threads into managed project context." }),
      ...
      capability({ key: "versioned_proposals", label: "Versioned proposal PDF workflow", domainKey: "proposals", matchPatterns: ["proposal.{0,30}(?:pdf|version)|versioned.{0,30}(?:pdf|proposal)|pdf.{0,30}proposal"], evidencePathPatterns: ["pdf_generator\\.py|ProposalViewer"], exampleClaim: "Delivered proposal PDF rendering with versioned storage and revision history." }),
  ...
  },
  {
    ...
    expectedCapabilities: [
      ...
      capability({ key: "artifact_generation", label: "Reviewed artifact generation", domainKey: "artifacts", matchPatterns: ["approved.{0,25}highlight.{0,35}artifact|artifact.{0,30}(?:resume|linkedin|highlight)|resume.{0,25}generat"], evidencePathPatterns: ["artifact-generation-service\\.ts"], exampleClaim: "Generates career artifacts from approved, provenance-backed highlights." }),
  ...
  },
  {
    ...
    expectedCapabilities: [
      capability({ key: "product_catalog", label: "Product catalog management", domainKey: "inventory", matchPatterns: ["product.{0,35}(?:detail|catalog).{0,35}(?:add|remove|view|manage|crud)|(?:add|remove|view|manage|crud).{0,35}product.{0,35}(?:detail|catalog)"], evidencePathPatterns: ["(?:model/ProductDetails(?:List)?|ui/PD(?:Add|Remove|View))\\.java"], exampleClaim: "Implemented ASIN-keyed product catalog management." }),
      capability({ key: "purchase_orders", label: "Purchase-order management", domainKey: "inventory", matchPatterns: ["purchase.?order.{0,35}(?:add|remove|view|track|manage)|(?:add|remove|view|track|manage).{0,35}purchase.?order"], evidencePathPatterns: ["(?:model/PurchaseOrders(?:List)?|ui/PO(?:Add|Remove|View))\\.java"], exampleClaim: "Implemented purchase-order tracking and management." }),
      capability({ key: "unit_economics_tracking", label: "Sales performance and unit-economics tracking", domainKey: "inventory", matchPatterns: ["(?:sales|product).{0,30}(?:performance|net revenue|net profit|cost per unit|unit economics)|(?:net revenue|net profit|cost per unit|unit economics).{0,30}(?:sales|product|order)"], evidencePathPatterns: ["model/ProductPerformance(?:List)?\\.java"] , exampleClaim: "Tracked quantity sold, net revenue, unit cost, and calculated net profit per order." }),
