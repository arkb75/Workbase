...
export const repositoryKnowledgeFixtures = [
  ...
  },
  {
    ...
    expectedCapabilities: [
      capability({ key: "account_session", label: "Account and signed session flow", domainKey: "access", matchPatterns: ["(?:account|signup|login|sign.?in).{0,50}(?:session|cookie|auth)|signed.{0,20}session"], evidencePathPatterns: ["src/app/api/v1/auth/|src/lib/session\\.ts"], exampleClaim: "Implemented account signup and login with a signed cookie-backed session." }),
