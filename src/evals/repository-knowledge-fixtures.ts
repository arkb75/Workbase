...
export const repositoryKnowledgeFixtures = [
  ...
  },
  {
    ...
    expectedCapabilities: [
      ...
      capability({ key: "rest_api", label: "Dataset REST API", domainKey: "experience", matchPatterns: ["rest.{0,20}(?:api|endpoint)|(?:get|put|delete|post).{0,30}(?:dataset|query)|dataset.{0,20}api", "(?=[\\s\\S]*\\bhttp\\b)(?=[\\s\\S]*\\b(?:api|endpoint)\\b)(?=[\\s\\S]*\\b(?:dataset|query)\\b)"], evidencePathPatterns: ["src/rest/Server\\.ts"], exampleClaim: "Exposed dataset lifecycle and query execution through a REST API." }),
      capability({ key: "visual_insights", label: "Interactive insight visualization", domainKey: "experience", matchPatterns: ["(?:visual|chart|graph).{0,30}(?:insight|dataset)|(?:insight|dataset).{0,30}(?:visual|chart|graph)|interactive.{0,25}insight"], evidencePathPatterns: ["components/Insights\\.js"], exampleClaim: "Delivered parameterized insights with interactive chart visualization." }),
