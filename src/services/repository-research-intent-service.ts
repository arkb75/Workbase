const explicitLiveRepositoryActionPattern =
  /(?:\b(?:please\s+)?(?:pull|inspect|search|read|check|access|look(?:\s+at)?)\b.{0,100}\b(?:repo|repository|github|codebase)\b)|(?:\b(?:can|could|would|will)\s+you\s+(?:please\s+)?(?:pull|inspect|search|read|check|access|look(?:\s+at)?)\b.{0,100}\b(?:repo|repository|github|codebase)\b)|(?:\b(?:inspect|search|read|check|access|compare)\b.{0,100}\b[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\b)/i;
...
const explicitRepositoryRefreshActionPattern =
  /(?:^|[.!?]\s+)(?:please\s+)?refresh\s+(?:(?:the|this|my|our)\s+)?(?:repo|repository|codebase)\b|\b(?:can|could|would|will)\s+you\s+(?:please\s+)?refresh\s+(?:(?:the|this|my|our)\s+)?(?:repo|repository|codebase)\b|\b(?:run|start|perform|trigger)\b.{0,50}\b(?:repo|repository|codebase)(?:\s+knowledge)?\s+refresh\b/i;
