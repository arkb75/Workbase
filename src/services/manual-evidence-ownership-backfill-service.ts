...
function hasTrustedLegacySourceShape(source: {
  ...
  return source.createdAt <= LEGACY_MANUAL_EVIDENCE_OWNERSHIP_CUTOFF &&
...
function hasTrustedInterimEvidenceMetadata(value: unknown, lineIndex: number) {
  const metadata = metadataRecord(value);
  if (!metadata) return false;
  return metadata.kind === USER_AUTHORED_MANUAL_NOTE_KIND &&
    metadata.userAuthored === true &&
    metadata.ownershipPolicyVersion ===
      USER_AUTHORED_MANUAL_NOTE_POLICY_VERSION &&
    metadata.lineIndex === lineIndex &&
    metadata.sourceType === "manual_note";
}
    ...
    const candidates = source.evidenceItems.flatMap((evidence) => {
      ...
      if (
        ...
        !(
          ...
          (
            legacySource &&
            evidence.createdAt <= LEGACY_MANUAL_EVIDENCE_OWNERSHIP_CUTOFF &&
            hasTrustedInterimEvidenceMetadata(evidence.metadata, lineIndex)
          )
        )
      ) {
        return [];
