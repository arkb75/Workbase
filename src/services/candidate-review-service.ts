...
export async function resolveAgentCandidate(
  ...
  if (editedText && (editedText.length < 10 || editedText.length > 240)) {
    throw new Error("Approved highlight text must contain between 10 and 240 characters.");
  }
  ...
  if (candidate.kind === "highlight_revision" && candidate.highlightSuggestion) {
