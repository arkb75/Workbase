  ...
  const selection = await runAuditedStructuredGeneration({
    ...
    execute: () => getStructuredLlmClient("deep_synthesis").generateStructured({
      systemPrompt: [
        ...
        "Calibrate centrality to the repository's actual product scope, not to the complexity of a larger hypothetical system. In a focused application, an implemented domain create, update, remove, or review flow can be a central outcome even when its code is modest.",
        "Inspect every distinct operation community before finishing. When a community represents a qualifying user or system goal, select its strongest grounded representative; this is an absolute relevance test, not a quota, and communities containing only routine support still contribute nothing.",
        "When candidates describe layers of the same operation, prefer the one that names the concrete domain action or state transition. Omit aggregate save plumbing, serialization, navigation, or display mechanics when a more meaningful candidate represents that operation, but do not use those mechanics as a substitute for a distinct qualifying domain workflow.",
